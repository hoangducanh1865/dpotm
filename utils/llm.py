...
class LLM:
    def __init__(self, current_run_dir, num_top_words, vocab, args):
        load_dotenv()
        ...
        self.generate_topic_descriptions_method = args.generate_topic_descriptions_method
    ...
    def generate_topic_word_preference_dataset(self):
        ...
        elif self.topic_word_dataset_generate_method == 'hf':
            # @TODO
            pass
    ...
    def generate_doc_topic_preference_dataset(self):
        ...
        if self.doc_topic_dataset_generate_method == 'hf':
            self.__setup_hf_llm()
        elif self.doc_topic_dataset_generate_method == 'openai':
        ...
        num_docs_per_call = self.num_docs_per_call
        doc_topic_preference_dataset = []
        failed_doc_count = 0
        
        for batch_start in range(batch_continue, theta.shape[0], num_docs_per_call):
            batch_end = min(batch_start + num_docs_per_call, theta.shape[0])
            batch_docs = []
            ...
            try:
                ...
                if self.doc_topic_dataset_generate_method == 'hf':
                    ...
                    print('Generate doc topic preference dataset not supported, fallback to OpenAI API method')
                ...
                for i, doc_data in enumerate(batch_docs):
                    ...
                    if numbers:
                        ...
                        if len(ranking) != 5:
                            print('⚠️ Different from 5 numbers found in this line, use theta order instead')
                            ranking = top_5_topic_indices.tolist()
                            failed_doc_count += 1
                        else:
                            print(f'Doc {doc_idx} extracted ranking: {ranking}')
                    else:
                        print(f'⚠️ No numbers found for doc {doc_idx}, use theta order instead')
                        ranking = top_5_topic_indices.tolist()
                        failed_doc_count += 1
    ...
    def generate_topic_descriptions(self):
        ...
        """
        if self.generate_topic_descriptions_method == 'openai':
        ...
        elif self.generate_topic_descriptions_method == 'hf':
            # @TODO
            pass
