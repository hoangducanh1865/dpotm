    ...
    def train(self, dataset_handler, verbose=False):
        ...
        for epoch in tqdm(range(1, self.epochs + 1)):
            ...
            for batch_data in dataset_handler.train_dataloader:
                ...
                rst_dict = self.model(
                    batch_data["data"], batch_data["contextual_embed"]
                )
                batch_loss = rst_dict["loss"]
        ...
        self.train_theta = self.model.get_theta(
            self.train_simple_embedding, self.train_simple_embedding
        )
        end_train_inference = time()
        print("*******************")
        print(
            "Train inference time: " + str(end_train_inference - start_train_inference)
        )
        print("*******************")
...
class WeteBasicTrainer:
    ...
    def to_list(self, data, device="cuda:0"):
        ...
        for i in range(len(data)):
            ...
            data_list.append(
                torch.tensor([j for j in idx for _ in range(data[i, j])], device=device)
            )
        return data_list
    ...
    def make_optimizer(
        ...
        optimizer = torch.optim.Adam(
            self.trainable_params, lr=self.learning_rate, weight_decay=1e-3
        )
        return optimizer
            ...
            for batch_data in dataset_handler.train_dataloader:
                ...
                for p in self.trainable_params:
                    try:
                        p.grad = p.grad.where(
                            ~torch.isnan(p.grad),
                            torch.tensor(0.0, device=p.grad.device),
                        )
                        p.grad = p.grad.where(
                            ~torch.isinf(p.grad),
                            torch.tensor(0.0, device=p.grad.device),
...
class CTMBasicTrainer:
    ...
    def export_theta(self, dataset_handler):
        train_theta = self.test(
            dataset_handler.train_data, dataset_handler.train_contextual_embed
        )
        test_theta = self.test(
            dataset_handler.test_data, dataset_handler.test_contextual_embed
        )
        return train_theta, test_theta
